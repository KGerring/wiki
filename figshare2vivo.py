                ...
                if len(author["additional_name"]) > 0:
                    g.add(
                        (
                            ...
                            Literal(author["additional_name"]),
                        )
                    )
...
def make_figshare_rdf(work):
    ...
    if "defined_type" in work:
        try:
            g.add(
                (uri, RDF.type, type_map[-1 + work["defined_type"]])
            )  # Python is zero-based, Figshare is not
        except IndexError:
            g.add(
                (uri, RDF.type, BIBO.InformationResource)
            )  # If unknown, just add the parent type
...
def _main():
    ...
    for figshare_work in works_2018 + works_18 + works_h2018 + works_h18:
        ...
        article = get_figshare_article(str(figshare_work["id"]))
        if (
            "vivo2018" in [x.lower() for x in article["tags"]]
